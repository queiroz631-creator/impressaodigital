-- Corrige condição de corrida: duas mensagens simultâneas do mesmo telefone
-- podiam criar duas conversas (cada uma travando/respondendo independente).
-- Garante no máximo uma conversa por telefone (a mesma linha é reaberta a
-- cada novo atendimento) e permite usar upsert atômico no webhook.

-- Remove eventuais duplicatas históricas antes de criar a constraint,
-- mantendo a conversa mais recente por telefone e movendo mensagens/arquivos
-- das mais antigas para ela.
DO $$
DECLARE
  r RECORD;
  manter uuid;
