...
-- Reaproveita sorteio_premios, sorteio_cupons, sorteio_participantes,
-- sorteio_ganhadores e sorteio_auditoria. Nada aqui altera notas, saldo,
-- fontes, contribuições, geração/cancelamento de cupons ou o fechamento.

ALTER TABLE public.sorteio_ganhadores
  ADD COLUMN IF NOT EXISTS unidade integer,
  ADD COLUMN IF NOT EXISTS usuario_id uuid;

-- No máximo um ganhador por unidade do prêmio; múltiplas unidades continuam
-- permitidas (prêmio com quantidade 2, 3, 10...).
CREATE UNIQUE INDEX IF NOT EXISTS sorteio_ganhadores_unidade_unica
  ON public.sorteio_ganhadores (sorteio_id, premio_id, unidade);
...
DECLARE
  ...
  v_numeros        jsonb;
BEGIN
  SELECT id, nome, numero_sorteio, status
    INTO v_sorteio
  ...
  -- Urna: cupons não cancelados de participantes que concorrem.
    ...
    FROM (
      SELECT p.ordem,
             ...
             jsonb_build_object(
               ...
               'ativo', p.ativo,
               'sorteados', (SELECT count(*) FROM public.sorteio_ganhadores g
                              WHERE g.sorteio_id = _sorteio_id AND g.premio_id = p.id),
               'disponivel', (SELECT count(*) FROM public.sorteio_ganhadores g
                               WHERE g.sorteio_id = _sorteio_id AND g.premio_id = p.id) < p.quantidade
             ) AS item
        FROM public.sorteio_premios p
  ...
  -- Amostra apenas para o efeito visual da animação (não decide nada).
  ...
  RETURN jsonb_build_object(
    ...
    'totais', jsonb_build_object(
      ...
      'premios_cadastrados', (SELECT count(*) FROM public.sorteio_premios p
                               WHERE p.sorteio_id = _sorteio_id AND p.ativo),
...
BEGIN
  -- 1) trava o sorteio: nenhuma outra apuração avança em paralelo.
  SELECT id, nome, numero_sorteio, status
    INTO v_sorteio
    FROM public.sorteios
   WHERE id = _sorteio_id
  ...
  -- 2) somente sorteio encerrado é apurado.
  ...
  -- 3) primeiro prêmio ativo com unidade livre, na ordem cadastrada.
  SELECT p.id, p.nome, p.descricao, p.quantidade, p.ordem
    INTO v_premio
    FROM public.sorteio_premios p
   WHERE p.sorteio_id = _sorteio_id
     AND p.ativo
     AND (SELECT count(*) FROM public.sorteio_ganhadores g
           WHERE g.sorteio_id = _sorteio_id AND g.premio_id = p.id) < p.quantidade
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('resultado', 'IGNORADO', 'motivo', 'sem_premio_disponivel',
                              'status', v_sorteio.status);
  END IF;

  -- 4) unidade exata a apurar: menor número livre de 1 até a quantidade.
  ...
  IF v_unidade IS NULL THEN
    RETURN jsonb_build_object('resultado', 'IGNORADO', 'motivo', 'unidade_ja_sorteada',
                              'premio_id', v_premio.id);
  END IF;

  -- 5/6) urna: cupons não cancelados, de participantes que concorrem e que
  -- ainda não ganharam nada neste sorteio (regra pelo participante).
  SELECT c.id, c.numero, c.participante_id
    INTO v_cupom
    FROM public.sorteio_cupons c
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('resultado', 'IGNORADO', 'motivo', 'sem_cupom_elegivel',
                              'premio_id', v_premio.id, 'unidade', v_unidade);
  END IF;
  ...
  -- 7) registro do ganhador (índice único protege a unidade).
  ...
  VALUES
    (_sorteio_id, v_premio.id, v_cupom.participante_id, v_cupom.id, v_cupom.numero,
     v_unidade, _usuario_id, v_agora)
  RETURNING id INTO v_ganhador_id;

  -- 8) status: SORTEADO somente quando não restar unidade dos prêmios ativos.
  SELECT coalesce(sum(p.quantidade), 0)
  ...
  IF v_restantes <= 0 THEN
    UPDATE public.sorteios
       SET status = 'SORTEADO',
           data_sorteio = coalesce(data_sorteio, v_agora)
     WHERE id = _sorteio_id
