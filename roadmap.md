...
- [x] Enviar clientes Sistema → Lojamix com criação de cadastro novo (vincular por CPF, criar entidade+pessoa_fisica em transação, modo simulação, marcador ultimo_id_cliente_pendente circular, elegibilidade = PF + CPF válido + telefone + participação em sorteio ATIVO; fluxo de notas intacto) — em andamento
