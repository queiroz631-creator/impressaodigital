...
SELECT TOP ({{limite}})
 ...
 WHERE REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(pf.cpf)), '.', ''), '-', ''), '/', ''), ' ', '') = {{cpf}}""",
    ...
    "cliente_criar_entidade": {
        ...
        "tipo": "write",
        "descricao": "Cria a entidade do cliente novo no Lojamix espelhando o cadastro feito pela tela do Lojamix e DEVOLVE o id_entidade (obrigatório OUTPUT INSERTED.id_entidade).",
        "placeholders": ["nome", "email", "ddd", "numero"],
        "sql": """INSERT INTO dbo.entidade (
    nome,
    email_principal,
    celular_ddd,
    celular_numero,
    ...
    valor_pontuacao,
    quantidade_pontos,
    valor_faixa_pontuacao,
    valor_sobra_acumulado,
    id_transportadora_padrao,
    valor_limite_compra,
    periodo_limite_compra,
