/** Processamento do webhook da Z-API. Somente servidor. */
import { z } from "zod";
...
export async function processarWebhookWhatsapp(request: Request, token: string): Promise<Response> {
  ...
  const ehGrupo =
    corpo.isGroup === true || Boolean(corpo.participantPhone) || /@g\.us$/i.test(corpo.phone ?? "");
  ...
  const phoneEhLid =
    /@lid$/i.test(corpo.phone ?? "") || String(corpo.phone ?? "").replace(/\D/g, "").length > 15;
