...
**Migração**
- Nova `public.sorteio_sincronizacao_fila`: `id uuid`, `sequencia bigserial` (cursor), `tipo text`, `entidade text`, `entidade_id text`, `sorteio_id uuid NULL REFERENCES sorteios(id)`, `origem text`, `destino text`, `operacao text`, `operacao_id text`, `status text default 'PENDENTE'`, `tentativas int default 0`, `ultima_tentativa_em`, `processado_em`, `erro text`, `payload jsonb NULL`, `criado_em`, `atualizado_em` + trigger `sorteio_set_atualizado_em`. Índice único `(origem, entidade, operacao_id)` (parcial, onde `operacao_id` não é nulo), índices `(status, sequencia)` e `(tipo, status)`. GRANT: `select` para `authenticated` (observabilidade), `all` para `service_role`; RLS habilitada com política de leitura via `pode_sorteios()`.
- `public.sorteio_sincronizacoes`: colunas `lote_id text NULL`, `operacao_id text NULL`, `sorteio_id uuid NULL REFERENCES sorteios(id)`, `origem text NULL`, `destino text NULL`, `duracao_ms int NULL`, `atualizado_em timestamptz default now()`; índice `(tipo, status, iniciado_em desc)`. Nenhuma coluna existente é alterada ou removida.
- `public.sorteio_sincronizacao_cursores`: `consumidor text primary key`, `sequencia bigint not null default 0`, `atualizado_em`. Guarda o cursor confirmado por consumidor (API local), independente do relógio local.
- `public.clientes`: `origem_id text NULL` (identificador estável da loja) + índice único parcial, e `origem_alteracao text NULL` para marcar a última origem da alteração (evita eco).
