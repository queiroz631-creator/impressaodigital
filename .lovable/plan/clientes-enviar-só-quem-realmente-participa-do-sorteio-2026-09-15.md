...
- `app/repositories/clientes_repo.py`: nova `maior_id_nota()` — `SELECT MAX(id_nota_fiscal) FROM dbo.nota_fiscal` — capturada uma vez no início do ciclo, servindo de `ate_id_nota` da segunda passagem (nunca o cursor das notas).
- `app/services/clientes.py`: `enviar_para_sistema` consulta o sorteio ativo uma vez, obtém o período, captura `ate_id_nota` via `maior_id_nota()` uma única vez no início do ciclo e faz as duas passagens (cadastro novo por `id_entidade`; clientes das notas na faixa `ultimo_id_nota_cliente < id_nota_fiscal <= ate_id_nota`), descarta em Python quem não tem CPF válido, telefone válido ou nome, envia em lote e só avança cada marcador após o lote aceito — erro/timeout mantém os marcadores. `ultimo_id_nota` não é lido nem gravado aqui. Sem sorteio ativo, o ciclo termina sem enviar nada.
