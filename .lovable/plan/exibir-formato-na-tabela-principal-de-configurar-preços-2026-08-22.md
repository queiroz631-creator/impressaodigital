...
4. **Remover o campo "Formato" do modal "Editar"** (o bloco `Select` de formato dentro do `DialogContent`), já que agora fica visível/editável direto na tabela. O `salvarUm`/`salvarMaterial` continua persistindo `formato`.
5. O `Select`/`SelectItem`/`SelectContent` já estão importados; `FORMATOS` e `FormatoPapel` também já vêm de `@/lib/calc`. Sem novas dependências.
